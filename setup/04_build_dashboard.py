...
dashboard = {
    "datasets": [
        ...
        {"name": "ds_accounts", "displayName": "Capital accounts",
         "queryLines": ["SELECT file_name, fund_name, closing_balance, currency FROM capital_accounts"],
         "columns": [{"displayName": "LP NAV", "expression": "SUM(`closing_balance`)"}]},
        ...
        {"name": "ds_docs", "displayName": "Documents by fund",
         "queryLines": ["SELECT split(file_name,'__')[0] AS fund, doc_type FROM capital_calls\n",
                        ...
                        "UNION ALL\n",
                        "SELECT split(file_name,'__')[0] AS fund, doc_type FROM capital_accounts"]},
    ...
    "pages": [{
        ...
        "layout": [
            {"widget": {"name": "header", "multilineTextboxSpec": {"lines": [
                ...
                "AI-extracted, deterministically-validated data from **91 fund-administration PDFs** ",
                "(capital calls + distribution notices + LP capital-account statements, 7 funds, 3 currencies). ",
                "The model extracts; arithmetic **validation gates** it — the anomalies table (bottom) ",
            ...
            {**counter("kpi_calls", "ds_calls", "calls", "COUNT(`file_name`)", "Capital calls"),
             "position": pos(0, 3, 2, 3)},
            {**counter("kpi_dists", "ds_dist", "dists", "COUNT(`file_name`)", "Distributions"),
             "position": pos(2, 3, 2, 3)},
            {**counter("kpi_accounts", "ds_accounts", "accounts", "COUNT(`file_name`)", "Capital accounts"),
             "position": pos(4, 3, 2, 3)},
            {**counter_measure("kpi_usd", "ds_calls", "USD called", "USD capital called", USD_FMT),
             "position": pos(6, 3, 2, 3)},
            {**counter_measure("kpi_nav", "ds_accounts", "LP NAV", "LP NAV tracked", USD_FMT),
             "position": pos(8, 3, 2, 3)},
            {**counter_measure("kpi_anom", "ds_validation", "Anomalies", "Anomalies flagged"),
             "position": pos(10, 3, 2, 3)},
            ...
            {**bar("bar_nav", "ds_accounts", "fund_name", "`fund_name`", "nav", "SUM(`closing_balance`)",
