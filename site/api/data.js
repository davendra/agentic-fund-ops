  ...
  try {
    ...
    const [kpi, cur, fund, dist, trend, rank, anom, nav] = await Promise.all([
      ...
      q(`SELECT split(file_name,'__')[0] fund, doc_type, count(*) FROM (
            ...
            UNION ALL SELECT file_name,doc_type FROM ${T}.distributions
            UNION ALL SELECT file_name,doc_type FROM ${T}.capital_accounts) GROUP BY fund,doc_type ORDER BY fund`),
      ...
      q(`SELECT fund_name, sum(closing_balance) FROM ${T}.capital_accounts
    ...
    const data = {
      ...
      kpis: { capital_calls: num(kpi[0][0]), distributions: num(kpi[0][1]), capital_accounts: num(kpi[0][2]),
        usd_called: num(kpi[0][3]), nav: num(kpi[0][4]), anomalies: num(kpi[0][5]),
        funds: 7, currencies: 3, gold_docs: fallback.eval.gold_docs },
      ...
      nav_by_fund: nav.map((r) => ({ fund: r[0], nav: num(r[1]) })),
